...
class PaymentMethodTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->objectManager->addSharedInstance($this->baseLogger->reveal(), LoggerInterface::class);
    }
    ...
    public function testHandlePositiveNotifyRegistersCaptureNotification()
    {
        ...
        $this->assertMessageLogged(
            Monolog::DEBUG,
            sprintf('Currently handling notify for order "#%s"...', $orderIncrementId)
        );
        $this->assertMessageLogged(Monolog::DEBUG, 'Transaction result is positive');
        $this->assertMessageLogged(
            Monolog::DEBUG,
            sprintf('Payment action is "%s"', AbstractMethod::ACTION_AUTHORIZE_CAPTURE)
        );
        $this->assertMessageLogged(
            Monolog::DEBUG,
            sprintf('Registered capture notification for order "%s"', $orderIncrementId)
        );
    }
    ...
    public function testHandleNegativeNotifyDoesNotRegistersCaptureNotification()
    {
        ...
        $this->assertMessageLogged(Monolog::DEBUG, 'Transaction result is negative');
    }
    ...
    public function testHandleNegativeNotifyCancelOrderIfConfigured()
    {
        ...
        $this->assertMessageLogged(Monolog::DEBUG, sprintf('Order "#%s" has been canceled', $orderIncrementId));
    }
    ...
    public function testHandlePositiveAuthNotifyRegistersAuthorizationNotification()
    {
        ...
        $this->assertMessageLogged(Monolog::DEBUG, 'Transaction result is positive');
        $this->assertMessageLogged(Monolog::DEBUG, sprintf('Payment action is "%s"', AbstractMethod::ACTION_AUTHORIZE));
        $this->assertMessageLogged(
            Monolog::DEBUG,
            sprintf('Registered authorization notification for order "%s"', $orderIncrementId)
        );
    }
     ...
     * @param $message
     */
    private function assertMessageLogged($level, $message)
    {
        /** @var Logger $logger */
        $logger = $this->objectManager->get(LoggerInterface::class);
        $this->assertContains(
            ['level' => Monolog::getLevelName($level), 'message' => $message],
            $logger->getMessages()
        );
    }
}
